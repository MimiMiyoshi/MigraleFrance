      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-green-600">
                      {stats.tasksStats.completed}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>未完了タスク</span>
                    <span className="text-amber-600">
                      {stats.tasksStats.incomplete}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>期限切れ</span>
                    <span className="text-red-600">
                      {stats.tasksStats.overdue}
                    </span>
                  </div>
                </div>
              </>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            {stats?.tasksStats &&
            stats.tasksStats.dueToday +
              stats.tasksStats.dueTomorrow +
              stats.tasksStats.dueThisWeek >
              0 ? (
              <div className="space-y-3">
                ...
                {stats.tasksStats.dueTomorrow > 0 && (
                  <div className="flex items-center justify-between pb-2 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <span className="font-medium">
                      {stats.tasksStats.dueTomorrow}件
                    </span>
                  </div>
                )}
                {stats.tasksStats.dueThisWeek > 0 && (
                  <div className="flex items-center justify-between pb-2 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <span className="font-medium">
                      {stats.tasksStats.dueThisWeek}件
                    </span>
                  </div>
                )}
                {stats.tasksStats.dueThisMonth > 0 && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium">
                      {stats.tasksStats.dueThisMonth}件
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div className="space-y-3">
              <Link
                href="/questionnaire"
                className="block w-full py-2 px-4 bg-blue-100 dark:bg-blue-900 hover:bg-blue-200 dark:hover:bg-blue-800 text-blue-800 dark:text-blue-200 rounded-md transition-colors"
              >
                ビザ診断を受ける
              </Link>
              <Link
                href="/tasks"
                className="block w-full py-2 px-4 bg-green-100 dark:bg-green-900 hover:bg-green-200 dark:hover:bg-green-800 text-green-800 dark:text-green-200 rounded-md transition-colors"
              >
                タスクを管理する
              </Link>
              {stats?.visaStats?.latestResult && (
                <Link
                  href="/visa-result"
                  className="block w-full py-2 px-4 bg-purple-100 dark:bg-purple-900 hover:bg-purple-200 dark:hover:bg-purple-800 text-purple-800 dark:text-purple-200 rounded-md transition-colors"
                >
                  ビザ結果を確認する
                </Link>
              )}
